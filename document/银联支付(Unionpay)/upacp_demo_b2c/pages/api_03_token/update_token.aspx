<form class="api-form" method="post" action="demo/api_03_token/Form_6_5_UpdateToken.aspx" target="_blank">
<p>
<label>商户号：</label>
...
<p>
  <label>订单发送时间：</label>
  <input id="txnTime" pattern="\d{14}" type="text" name="txnTime" placeholder="订单发送时间" value="<%= DateTime.Now.ToString("yyyyMMddHHmmss") %>" title="取北京时间，YYYYMMDDhhmmss格式。如之前调用获取验证码接口，填写获取验证码时的orderId。" required="required"/>
</p>
<p>
  <label>商户订单号：</label>
...
<p>
  <label>Token：</label>
  <input id="token" pattern="[0-9]{13,19}" type="text" name="token" placeholder="token号" value="" title="token号，开通和开通查询接口银联应答。" required="required"/>
</p>
<p>
<label>&nbsp;</label>
