...
3） 此工程可以跑起来测试，建议使用chrome、firefox打开。
  全渠道PC网关/WAP支付产品消费交易示例地址为：http://IP：端口/index_16_qrc.aspx
───────────
...
upacp_demo_qrc
  ...
  │  ├生产环境配置文件
  │  │  │
  │  │  └Web.config.证书 ┈┈┈┈┈┈┈┈┈ 【重要】 生产环境配置文件样例（使用方式请看文件里的说明）
  │  │ 
  │  ├测试环境配置文件
  │  │  │
  │  │  └Web.config.证书 ┈┈┈┈┈┈┈┈┈ 【重要】 测试环境配置文件样例（使用方式请看文件里的说明）
  │  │ 
  │  │ 
  │  └对账文件样例 
  │     │
  ...
  │      │ 
  │      ├Form_6_2_ApplyQrCode.aspx ┈┈┈┈┈ 主扫申请二维码交易示例（后台） 
  │      │ 
  │      ├Form_6_2_Consume.aspx ┈┈┈┈┈ 被扫二维码消费示例（后台）
  │      │ 
  │      ├Form_6_3_ConsumeUndo.aspx ┈┈┈┈┈ 消费撤销示例类（后台）
  │      │ 
  │      ├Form_6_3_Query.aspx ┈┈┈┈┈ 交易状态查询示例类（后台）
  │      │ 
  │      ├Form_6_4_Refund.aspx ┈┈┈┈┈ 退货示例类（后台）
  │      │
  ...
  │
  └index_10_df.aspx ┈┈┈┈ demo演示入口页面

───────────
 **注意**

1.【接口规范】该接口参考文档位置：
      接口产品规范：open.unionpay.com帮助中心 下载  产品接口规范  《二维码接口规范与终端改造指南》
